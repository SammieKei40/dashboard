...
export default function Input(props) {
  ...
  const styleTypes = {
    input:
      "w-full text-black text-3xl border-2 border-gray-100 rounded-md focus:outline-none focus:ring-1 focus:ring-green-100 focus:border-green-100 placeholder-black transition",
  };
  ...
  const handleIconClick = () => {
    ...
    if (isHidden) {
      setIcon("O"); // Change the icon to "O" (open eye)
      setInputType("text");
    } else {
      setIcon("C"); // Change the icon back to "C" (closed eye)
  ...
  return (
    <div className={full ? "w-full" : ""}>
      ...
      {error && (
        <p className="font-medium mt-2 text-xs text-red-600">{error}</p>
      )}
    </div>
  );
}
